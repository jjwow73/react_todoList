...
import './TodoItem.css';
const makeTodoItem = ({text, checked, id, color, onToggle, onRemove}) => {
    console.log('TodoItem 만듦');
    return (
...
const TodoItem = ({text, checked, id, color, onToggle, onRemove}) => {
    const todo = useMemo(() => makeTodoItem({text, checked, id, color, onToggle, onRemove}), [text, checked, id, color, onToggle, onRemove]);
    return todo;
}
/*
